...
if not (config and config.SkipIncantations) then
    ...
    -- ============================================================================
    -- MIGRATION INCANTATION (For legacy users who want to use new progression)
    -- ============================================================================
    
    -- Migration incantation: allows users with the old BoonStacker_Unlock 
    -- to opt-in to the new progression system
    local migrationData = {
    ...
    -- Build requirements: only visible if user has old BoonStacker_Unlock and hasn't migrated
    local function BuildMigrationRequirements()
        return {
            {
                -- Must have the old legacy unlock
                PathTrue = { "GameState", "WorldUpgrades", "BoonStacker_Unlock" },
            },
            {
                -- Must NOT have already migrated
                PathFalse = { "GameState", "WorldUpgrades", "BoonStacker_Legacy_Migration" },
            },
        }
    end
    ...
    Incantations.addIncantation({
        ...
        FlavorText = migrationData.flavorText,
        WorldUpgradeData = {
            Icon = migrationData.icon,
            Cost = migrationData.cost,
            GameStateRequirements = BuildMigrationRequirements(),
            IncantationVoiceLines = {
                {
                    PreLineWait = 0.3,
                    { Cue = "/VO/Melinoe_1081", Text = "{#Emph}If Time cannot be stopped, then we shall see; {#Emph}If spill my blood I must, so mote it be...!" },
                },
            },
        },
        OnEnabled = function(source, incantationId)
            print("BoonStacker: Migration incantation activated via " .. source)
            if source == "purchase" then
                -- Only perform migration on actual purchase, not on save load
                if BoonStacker and BoonStacker.PerformLegacyMigration then
                    BoonStacker.PerformLegacyMigration()
                end
            end
        end,
    })
    print("BoonStacker: Registered migration incantation: " .. migrationData.id)
